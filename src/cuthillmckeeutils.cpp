...
int CuthillMckeeUtils::getBandWidth(const MatrixXb &adjancecyMatrix,
                                    const int currentBandWidth) {
  int bandWidth = 0, j = currentBandWidth - 1;
  while (j > 0 && !adjancecyMatrix.coeff(0, j))
    j--;
  bandWidth = j + 1;
  if (bandWidth != currentBandWidth) {
    for (int i = 1; i < adjancecyMatrix.rows(); ++i) {
      j = std::min((int)adjancecyMatrix.rows(), i + currentBandWidth) - 1;
      while (j > i && !adjancecyMatrix.coeff(i, j))
        j--;
      if (j - i >= bandWidth)
        bandWidth = j - i + 1;
      if (bandWidth == currentBandWidth)
...
vector<int>
CuthillMckeeUtils::selectFirstVertice(const MatrixXb &adjancecyMatrix,
  ...
  do {
    ...
    for (i = 0; i < visited.size() - 1; ++i) {
      ...
      for (j = v + 1; j <= v + currentBandWidth && j < adjancecyMatrix.rows();
           ++j) {
      ...
      if (visitedCount >= adjancecyMatrix.rows())
        break;
    }
...
vector<int> CuthillMckeeUtils::ReverseAlgorithm(MatrixXb &adjancecyMatrix,
                                                std::vector<int> &degrees,
                                                int currentBandWidth) {
  vector<int> order =
      selectFirstVertice(adjancecyMatrix, currentBandWidth, degrees);
...
MatrixXb CuthillMckeeUtils::adjancecyMatrixFromPointLinks(
  ...
  for (int rowIndex = 0; rowIndex < adjancecyMatrix.rows(); ++rowIndex)
    for (int linkIndex = 0; linkIndex < points[rowIndex].links.size();
         ++linkIndex)
      adjancecyMatrix.coeffRef(rowIndex, points[rowIndex].links[linkIndex]) =
          true;
