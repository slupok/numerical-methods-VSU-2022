...
float TriangulationUtils::AngleBetween(PointWithLinks vector1, PointWithLinks vector2)
{
...
std::pair<int, float> TriangulationUtils::findMinAngle(const std::vector<PointWithLinks> &localPoints, const std::vector<PointWithLinks> &points)
{
    ...
    for (int i = 0; i < localPoints.size(); i++)
    {
        ...
        PointWithLinks prevPoint = PointWithLinks(points[indexOfPrev].x - currentPoint.x, points[indexOfPrev].y - currentPoint.y);
        PointWithLinks nextPoint =  PointWithLinks(points[indexOfNext].x - currentPoint.x, points[indexOfNext].y - currentPoint.y);
...
std::vector<Triangle> TriangulationUtils::Triangulate(std::vector<PointWithLinks> &points, float k, float mainAngle, float rightAngle, float auxiliaryAngle)
{
    ...
    while (localPoints.size() > 3)
    {
        ...
        else
        {
            ...
            PointWithLinks prevPoint = PointWithLinks(localPoints[indexOfPrev].x - localPoints[indexOfNext].x, localPoints[indexOfPrev].y - localPoints[indexOfNext].y);
            PointWithLinks nextPoint = PointWithLinks(localPoints[index].x - localPoints[indexOfNext].x, localPoints[index].y - localPoints[indexOfNext].y);

            a2 = AngleBetween(prevPoint, nextPoint);
            a3 = 180 - a2 - a1;

            if ((a1 > mainAngle && a1 <= rightAngle) && a2 >= auxiliaryAngle && a3 >= auxiliaryAngle)
            {
                Removal(points, localPoints, result, index);
            }
    ...
    int firstPoint = FindLastByCurrentNumber(points, localPoints[0].currentNumber);
    int secondPoint = FindLastByCurrentNumber(points, localPoints[1].currentNumber);
    int thirdPoint = FindLastByCurrentNumber(points, localPoints[2].currentNumber);
...
void TriangulationUtils::Removal(std::vector<PointWithLinks> &points, std::vector<PointWithLinks> &localPoints, std::vector<Triangle> &triangles, int index)
{
    ...
    localPoints[indexOfPrev].links.erase(localPoints[indexOfPrev].links.begin() + 1);
    localPoints[indexOfPrev].links.insert(localPoints[indexOfPrev].links.begin() + 1, nextLastInPoints);

    localPoints[indexOfNext].links.erase(localPoints[indexOfNext].links.begin());
    localPoints[indexOfNext].links.insert(localPoints[indexOfNext].links.begin(), prevLastInPoints);
...
void TriangulationUtils::Alignment(std::vector<PointWithLinks> &points, std::vector<PointWithLinks> &localPoints, std::vector<Triangle> &triangles, int index, float k)
{
    PointWithLinks pointToRemove = PointWithLinks(localPoints[index]);
    int prevCurrentNumber = points[pointToRemove.links[0]].currentNumber;
    int nextCurrentNumber = points[pointToRemove.links[1]].currentNumber;
    ...
    PointWithLinks prevPoint = PointWithLinks(localPoints[indexOfPrev].x - localPoints[index].x, localPoints[indexOfPrev].y - localPoints[index].y);
    PointWithLinks nextPoint = PointWithLinks(localPoints[indexOfNext].x - localPoints[index].x, localPoints[indexOfNext].y - localPoints[index].y);
    ...
    int prevLastInPoints = FindLastByCurrentNumber(points, prevCurrentNumber);
    int nextLastInPoints = FindLastByCurrentNumber(points, nextCurrentNumber);
    int lastIndex = FindLastByCurrentNumberSkippingOne(points, pointToRemove.currentNumber);
    points[points.size() - 1].links.push_back(lastIndex);

    localPoints[indexOfPrev].links.erase(localPoints[indexOfPrev].links.begin() + 1);
    localPoints[indexOfPrev].links.insert(localPoints[indexOfPrev].links.begin() + 1, points.size() - 1);

    localPoints[indexOfNext].links.erase(localPoints[indexOfNext].links.begin());
    localPoints[indexOfNext].links.insert(localPoints[indexOfNext].links.begin(), points.size() - 1);
...
int TriangulationUtils::FindByCurrentNumber(std::vector<PointWithLinks> &localPoints, int index)
{
    int pos = -1;
...
int TriangulationUtils::FindLastByCurrentNumber(std::vector<PointWithLinks> &localPoints, int index)
{
    int pos = -1;
...
int TriangulationUtils::FindLastByCurrentNumberSkippingOne(std::vector<PointWithLinks> &localPoints, int index)
{
    int pos = index;
...
void TriangulationUtils::VisitEdge(int startVerticesCount, std::vector<Vector2> &oldVertices, std::vector<Vector2> &newVertices, std::vector<int> &newVerticesNeighbors, int point1, int point2)
{
...
void TriangulationUtils::Relax(int startVerticesCount, std::vector<Vector2> &vertices, std::vector<int> &triangles)
{
    ...
    {
        VisitEdge(startVerticesCount, vertices, newVertices, newVerticesNeighbors, triangles[i], triangles[i + 1]);
        VisitEdge(startVerticesCount, vertices, newVertices, newVerticesNeighbors, triangles[i + 1], triangles[i + 2]);
        VisitEdge(startVerticesCount, vertices, newVertices, newVerticesNeighbors, triangles[i + 2], triangles[i]);
    }
    for (int i = 0; i < newVertices.size(); ++i)
    {
        vertices[i + startVerticesCount].x = newVertices[i].x / newVerticesNeighbors[i];
        vertices[i + startVerticesCount].y = newVertices[i].y / newVerticesNeighbors[i];
    }
}

PointWithLinks TriangulationUtils::medianVector(PointWithLinks pointA, PointWithLinks pointB, float k)
{
...
std::pair<QVector<int>, QVector<QPointF> > TriangulationUtils::triangulationPolygon(const QVector<QPointF> &points, const int relaxationStep)
{
    vector<PointWithLinks> pointsWidthLinks;

    for (int pointIndex = 0; pointIndex < points.size(); ++pointIndex)
        pointsWidthLinks.push_back(PointWithLinks(points[pointIndex].x(), points[pointIndex].y(), pointIndex, points.size()));

    vector<Triangle> triangles = TriangulationUtils::Triangulate(pointsWidthLinks, 1.0f, 75.0f, 90.0f, 30.0f);
    ...
    for (int relaxationIndex = 0; relaxationIndex < relaxationStep; ++relaxationIndex)
    ...
    for (int pointIndex = 0; pointIndex < pointsWidthLinks.size(); ++pointIndex)
        triangulationPoint.push_back(QPointF(pointsWidthLinks[pointIndex].x, pointsWidthLinks[pointIndex].y));

    return std::pair<QVector<int>, QVector<QPointF>>(indices, triangulationPoint);
}

std::vector<PointWithLinks> TriangulationUtils::pointsWithLinks(const QVector<QPointF> &points)
{
    vector<PointWithLinks> pointsWidthLinks;
