    ...
    : QMainWindow(parent)
{
    ...
    m_points << QPointF(0.0, 0.0) << QPointF(0.0, 1.0) << QPointF(1.0, 0.0) << QPointF(1.0, 1.0);
    m_indices << 0 << 1 << 2 << 1 << 2 << 3;

    // tmp
//    vector<PointWithLinks> pointsWithLink = pointsWithLinks(m_points);
//    MatrixXb adjacencyMatrix = adjancecyMatrixFromPointLinks(pointsWithLink);

//    vector<int> degrees = getDegrees(adjacencyMatrix);
//    vector<int> indices = ReverseAlgorithm(adjacencyMatrix, degrees, adjacencyMatrix.rows());
//    int currentLength = getBandWidth(adjacencyMatrix, adjacencyMatrix.cols());
    ...
    m_constraintsByAxis->addItems(QStringList() << "None" << "X" << "Y" << "XY");
    //
    ...
    connect(m_viewport, &Viewport::onPointSelected, this, &MainWindow::pointSelected);
    connect(m_constraintsByAxis, SIGNAL(currentIndexChanged(int)), this, SLOT(constraintsIndexChanged(int)));

    connect(m_uDisplacmentTextEdit, &QLineEdit::textChanged, this, &MainWindow::uDisplacmentChanged);
    connect(m_vDisplacmentTextEdit, &QLineEdit::textChanged, this, &MainWindow::vDisplacmentChanged);

    connect(m_applySettings, &QPushButton::pressed, this, &MainWindow::applySettings);
    connect(m_computeButton, &QPushButton::pressed, this, &MainWindow::computeDisplacment);
}
...
void MainWindow::pointSelected(const int pointIndex)
{
    ...
    for (int displacmentIndex = 0; displacmentIndex < m_displacments.size(); ++displacmentIndex)
    ...
    for (int constraintsIndex = 0; constraintsIndex < m_constraints.size(); ++constraintsIndex)
         if (m_constraints[constraintsIndex].pointIndex == m_selectedPointIndex) {
             StiffnessUtils::Constraints::Type constraintsType = m_constraints[constraintsIndex].type;
...
void MainWindow::uDisplacmentChanged(const QString &u)
{
    m_u = u.toFloat();
}

void MainWindow::vDisplacmentChanged(const QString &v)
{
    m_v = v.toFloat();
}
...
void MainWindow::applySettings()
{
    ...
    for (int constraintsIndex = 0; constraintsIndex < m_constraints.size(); ++constraintsIndex)
         if (m_constraints[constraintsIndex].pointIndex == m_selectedPointIndex) {
             m_constraints[constraintsIndex].type = StiffnessUtils::Constraints::Type(m_constraintsByAxis->currentIndex());
             return;
         }
    ...
    constraints.type = StiffnessUtils::Constraints::Type(m_constraintsByAxis->currentIndex());
...
{
    StiffnessUtils::compute(70000.0f, 0.3f, m_points, m_indices, m_displacments, m_constraints);
